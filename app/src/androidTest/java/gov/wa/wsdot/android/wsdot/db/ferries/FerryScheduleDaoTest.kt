...
class FerryScheduleDaoTest {
    ...
    fun insertAndReadAndDelete() {
        ...
        ferryScheduleDao.insertNewSchedules(listOf(schedule1))

        val scheduleById = ferryScheduleDao.loadSchedule(3).getOrAwaitValue()
        assertThat(scheduleById, notNullValue())
        assertThat(scheduleById, equalTo(schedule1))

        ferryScheduleDao.markSchedulesForRemoval()
        ferryScheduleDao.deleteOldSchedules()

        val nullSchedule = ferryScheduleDao.loadSchedule(3).getOrAwaitValue()

        assertThat(nullSchedule, nullValue())
